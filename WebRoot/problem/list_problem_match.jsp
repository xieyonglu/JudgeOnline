...
<input type="hidden" name="actionURL" value="problem_pageProblem.action?action=match&matchId=${match.matchId}"/>
...
<s:if test="#list.size()==0">
	没有试题信息，<a href="match_showMatch.action?matchId=${match.matchId}&action=problem">点击</a>添加！
</s:if>
<s:if test="#list.size()!=0">
    ...
    <tbody>
      <tr>
        <th align=center colspan=16 style="height: 23px">
        	<span style="color:red;"><b>${match.matchName}</b></span>比赛试题信息管理
        </th>
      </tr>
      <tr bgcolor="#DEE5FA">
      ...
      <s:iterator value="#list" status="st">
      	<tr bgcolor="#DEE5FA">
	        ...
	        <td width="10%" align=center class=txlrow>
	        	<a href="code_pageFindCode.action?action=admin&problemId=${problemId}&matchId=${match.matchId}">查看运行</a>
	        </td>
	        <td width="20%" align=center class=txlrow>
	        	...
	        	<img src="${pageContext.request.contextPath}/image/update.gif" width="16" height="16" />
					<a href="problem_findProblem.action?problemId=${problemId}&action=match">编辑</a>&nbsp;&nbsp;
				<img src="${pageContext.request.contextPath}/image/delete.gif" width="16" height="16" />
					<a href="problem_deleteMatchProblem.action?problemId=${problemId}&matchId=${match.matchId}">删除</a>
	        </td>
	    </tr>
	  </s:iterator>
      ...
      <tr bgcolor="#DEE5FA">
        <td colspan="16" align=center bgcolor="#DEE5FA" class=txlrow>
        	<span class="tablebody2">
          	...
          	<input type="button" value="添加" class="but" onclick="window.location.href='match_showMatch.action?action=problem&matchId=${match.matchId}'"/>
          	<input type="button" value="删除" class="but" onclick="batchDelete('problem_batchDeleteMatch.action?matchId=${match.matchId}')"/>
        </td>
      </tr>
