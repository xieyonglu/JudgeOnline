...
$(function(){
	...
	var pageSize=document.getElementsByName("select").item(0).value;
		...
		var str="code_pageFindCode.action?conditionA="+conditionA+"&conditionB="+conditionB+
				...
				"&action=admin&matchId="+${match.matchId}+"&problemId="+${problem.problemId};
...
<input type="hidden" name="actionURL" value="code_pageCode.action?matchId=${match.matchId}&action=admin"/>
...
<form name="form1" method="post" action="">
  <table width="1500" border="0" align=center cellpadding=2 cellspacing=1 bordercolor="#799AE1" class=tableBorder>
    <tbody>
      <tr>
        <th align=center colspan=16 style="height: 23px">${match.matchName}-${problem.title}用户的所有运行结果信息[管理员]</th>
      </tr>
      <tr bgcolor="#DEE5FA">
      ...
      <tr align="center" bgcolor="#799AE1">
        ...
        <td width="6%" align="center" class="txlHeaderBackgroundAlternate">运行号</td>
        <td width="10%" align="center" class=txlHeaderBackgroundAlternate>用户</td>
        <td width="10%" align="center" class=txlHeaderBackgroundAlternate>IP地址</td>
        <td width="15%" align="center" class=txlHeaderBackgroundAlternate>题目</td>
        <td width="6%" align="center" class=txlHeaderBackgroundAlternate>结果</td>
        <td width="5%" align="center" class=txlHeaderBackgroundAlternate>时间</td>
        <td width="5%" align="center" class=txlHeaderBackgroundAlternate>内存</td>
        <td width="5%" align="center" class=txlHeaderBackgroundAlternate>语言</td>
        <td width="5%" align="center" class=txlHeaderBackgroundAlternate>代码长度</td>
        <td width="10%" align="center" class=txlHeaderBackgroundAlternate>提交时间</td>
        <td width="12%" align="center" class=txlHeaderBackgroundAlternate>基本操作</td>
      </tr>
      ...
      <s:iterator value="#list" status="st">
      	<tr bgcolor="#DEE5FA">
      		...
      		<td width="5%" align="center" class="txlrow">
      			<input type="checkbox" name="checkList" value="${codeId}">
      		</td>
      		<td width="6%" align=center class=txlrow>
	        	<s:property value="#st.getIndex()+1+(pageBean.currentPage-1)*pageBean.pageSize"/>
	        </td>
	        <td width="6%" align=center class=txlrow>${runNumber}</td>
	        <td width="10%" align=center bgcolor="#DEE5FA" class=txlrow>${user.userName}</td>
	        <td width="10%" align=center bgcolor="#DEE5FA" class=txlrow>${ipaddress}</td>
	        <td width="15%" align=center class=txlrow>${problem.title}</td>
	        <td width="6%" align=center class=txlrow>${state}</td>
	        <td width="5%" align=center class=txlrow>${runtime}</td>
	        <td width="5%" align=center class=txlrow>${memory}</td>
	        <td width="5%" align=center class=txlrow>${language}</td>
	        <td width="5%" align=center class=txlrow>${codeLength}</td>
	        ...
	        <td width="12%" align=center class=txlrow>
				...
				<a href="code_showCode.action?codeId=${codeId}&action=show">查看</a>
				
				<img src="${pageContext.request.contextPath}/image/update.gif" width="16" height="16" />
				<a href="code_deleteCode.action?codeId=${codeId}">删除</a>
	        </td>
	  ...
	  <tr bgcolor="#DEE5FA">
        <td colspan="16" align=center bgcolor="#DEE5FA" class=txlrow>
        	<span class="tablebody2">
          	...
          	<input type="button" value="删除" class="but" onclick="batchDelete('code_batchDelete.action')"/>
        </td>
      </tr>
